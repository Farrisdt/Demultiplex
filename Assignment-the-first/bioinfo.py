...
#compliment_DNA = {A: T, T: A, C: G, G: C, U: A} #can be used with either RNA or DNA, produces DNA
#compliment_RNA = {A: U, U: A, C: G, G: C, T: A}
###Functions###
''' #needs to be finished
def reverse_compliment(line, RNAflag: bool=False) -> str:
    ''takes in a string of DNA or RNA, returns the reverse compliment''
    newline = ""
    if validate_base_seq(line):
        for char in range(len(line), -1, -1):
            newline+= bioinfo.reverse_compliment()
'''
def gc_content(DNA):
